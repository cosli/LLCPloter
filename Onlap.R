...
q3<-q2+geom_vline(x=sep[2:sumlayer,1],lty=2)
...
###仅保留SQmL2-SQmL1数据
p1<-ggplot(ONLAP,aes(编号,t))
p2<-p1+geom_point(aes(colour=层位))+geom_smooth(method="loess",span=0.1,se=TRUE)+coord_cartesian(xlim=c(18,36))+labs(title="SQmL2-SQmL1上超点纵向迁移图")+xlab("时间")+ylab("\u0394 t")
p3<-p2+geom_vline(x=sep[2:sumlayer,1],lty=2)
for (i in 1:length(levels(ONLAP[,2]))){
  p3<-p3+geom_text(x=mean(as.numeric(sep[i,1:2])),y=min(ONLAP[7]),hjust=0.5,vjust=1,label=sep[i,3],size=4)
}
